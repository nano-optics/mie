...
besselJ <- function(x,nu)
  as.vector(BesselJ(x, min(nu), nSeq = max(nu)+1 - min(nu)))

besselY <- function(x,nu)
  as.vector(BesselY(x, min(nu), nSeq = max(nu)+1 - min(nu)))
...
`Mie.coated` <-
function(m1, m2, x, y, order="all"){

if (x==y)     { # To reduce computing time
	result <- Mie(m1,y,order=order)
	return(result)
}                

if (x==0)     {      # To avoid a singularity at x=0
	result <- Mie(m2,y,order=order)
	return(result)
}
...
function(lambda, core.n, inner.radius = 0.04 , 
	...
	efficiencies <- mapply(Mie.coated,  m1=m1, m2 = m2, x = x, y = y, order=order)
