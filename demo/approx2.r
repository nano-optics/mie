...
params <- expand.grid(order = c(1, 2, Inf), radius = c(0.03, 0.05), material = c("gold", "silver"), 
...
pAu <- 
ggplot(mAu, aes(wavelength*1e3, value, colour=order, group=interaction(order, L1)))+
  facet_grid(variable~radius, scales="free") +
...
pAg <- pAu %+% mAg
library(gridExtra)
## grid.arrange(pAu, pAg)
pAg

## g <- arrangeGrob(pAu, pAg)

## ggsave("comparison.pdf", g, width=10, height=15)
